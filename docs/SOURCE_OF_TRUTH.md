...
**Seed-data / demo-narrative note — reconciled 2026-09-06:** `server/prisma/seed.js` now seeds the Netrix Systems / ZKTeco catalog and cast from `DEMO_SCENARIO.md` end to end, replacing the old Acme/Beta/Corner Shop placeholder. Specifics:

- **Catalog**: full Hardware (CCTV + ZKTeco SpeedFace/inBio260/K40/lock line) and Services tables from the doc, plus the three Software recurring plans (cloud storage, AI analytics, ZKBioTime) and AMC as a subscribable Service — all with `costPrice` seeded.
- **Categories**: restructured from the old 4-category nested tree to a flat 3-category tree (Hardware / Services / Software) matching the doc's two named ceilings plus a Software bucket. A real simplification (lost the old sub-category granularity), judged worth it since nothing in the demo needs it.
- **Tiers**: 3, not 4 — Gold/Silver/Bronze exactly as the doc names them. `PLATINUM` is deactivated (`isActive: false`), not deleted, so a shared dev DB with stale FKs into it never breaks; the scoring engine simply can never land a customer there anymore, which is functionally "3 tiers."
- **Customers**: Sundaram Textiles, Vistaar Financial, Kaveri Hospitals, Anand Motors, Rajdhani Logistics, with raw metrics hand-tuned against the live `TierScoringConfig` (not hardcoded tiers) until `recalculateAllTiers()` lands each one on the doc's named tier — verified: Sundaram 97.5 Gold, Vistaar 77.19 Gold, Kaveri 41.73 Silver, Anand 42.09 Silver, Rajdhani 9.56 Bronze.
- **Warehouses**: trimmed to the doc's exact 2 (Main Warehouse Bengaluru, Regional Depot Pune); the old 3rd (North Hub) is deactivated, not deleted. Camera stock (20 Bengaluru / 15 Pune) is deliberately short of the demo quotation's 32-unit line so the split is real — verified the demo quotation actually allocates 20 from Bengaluru + 12 from Pune.
- **The demo quotation** is seeded through the real `createQuotation`/`submitQuotation` service calls, not raw inserts — `evaluateDiscount()` genuinely runs and, verified against the live seeded DB, routes it to `PENDING_APPROVAL` / `ApprovalLevel.MANAGER` (blended severity ≈0.4, under Gold's finance-escalation threshold of 5 — matches the doc's own claim that this quote needs a Sales Manager, not Finance).
- **Full status spread** seeded across all 5 customers and both reps, entirely through real service calls (`createQuotation`, `submitQuotation`, `updateQuotationLines`, `actOnStep`, `withdrawQuotation`) plus a requote chain (`previousQuotationId`) from a `WITHDRAWN` quotation to a compliant `CONFIRMED` child. One exception: a quotation is pushed into `UNDER_NEGOTIATION` via a direct, audited `prisma.quotation.update` rather than a service call, because no service function performs that transition yet — the portal/`NegotiationMessage` counter-discount flow (§2.7/§1.4) genuinely isn't built. That gap is real, not a seed-data shortcut; it's the same gap already tracked below.
- **Subscriptions**: seeded via real quotation confirmation (`createSubscriptionsForQuotation`), with one driven to `CANCELLED` (`cancelSubscription`) and one to `PENDING_RENEWAL_APPROVAL` via a real `runBillingCycle()` tick. Note the real `SubscriptionStatus` enum has no `PAUSED` value (`ACTIVE` / `PENDING_RENEWAL_APPROVAL` / `PAST_DUE` / `CANCELLED`) — seeded to match the schema, not an earlier draft's wording.
- **Chat**: two seeded conversations/threads on real quotations (direct `prisma` writes — `ChatConversation`/`ChatMessage` are plain persisted rows with no invariant beyond FK validity, unlike the socket layer around them).
- Transactional rows (quotations, approvals, subscriptions, chat) are wiped and recreated on every seed run rather than upserted (no natural key for a workflow history); master/catalog data stays upsert-idempotent as before.
...
- [x] ~~4 test quotations (ids 1–4, customer "Beta Industries") exist in the shared dev DB~~ — **cleared**: the reconciled `seed.js` (2026-09-06) wipes all transactional data (quotations/approvals/subscriptions/chat) and the old placeholder customers on every run, before reseeding the real Netrix demo data.
- [x] `CustomerTier.financeEscalationSeverity` thresholds (3/4/5 for Bronze/Silver/Gold, Platinum dropped) — checked by hand against the live seeded demo quotation: blended severity ≈0.4 for the flagship Sundaram quote, safely under Gold's threshold of 5, so it routes to Manager only, matching `DEMO_SCENARIO.md`. Not re-tuned beyond that — good enough for the demo as rehearsed, revisit if a different demo quote is added later.
- [x] ~~Seed data does not match `DEMO_SCENARIO.md`~~ — **reconciled 2026-09-06**, see §6a's seed-data note.
